...
from loss import BPRLoss, RegLoss, SoftmaxLoss, CompareLoss, InfoNCE
...
class Trainer(AbsIndTrainer):
    ...
    def _get_user_hist(self, hist_indexs, stage = 'test'):
        ...
        session_history = []
        cur_session_history = []
        hist_sessions_history = []
        hist_indexs = hist_indexs.tolist()
        if stage == 'test':
            for hist_index in hist_indexs:
                session,behavior,cur_session,hist_sessions = self.hist_test_dict[hist_index]
                user_history.append(behavior)
                session_history.append(session)
                cur_session_history.append(cur_session)
                hist_sessions_history.append(hist_sessions)
            ...
            cur_session_history=torch.tensor(cur_session_history) # len与hist_len相同
            hist_sessions_history=torch.tensor(hist_sessions_history)
            # 每个session有效长度, 形状（sample_num,session_num）
            hist_sessions_len2 = torch.sum(hist_sessions_history[:,:,:,0] !=0, axis=2)
            # 历史session个数, 形状（sample_num,）
            hist_sessions_len = torch.sum(hist_sessions_len2 > 0, axis=1)
            session_len = torch.where(session_len>0, session_len,session_len+1 )

        return session_history, session_len, user_history, hist_len, cur_session_history, hist_sessions_history,hist_sessions_len,hist_sessions_len2
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, loss):
        ...
        des = '%.' + str(des) + 'f'
        train_loss_output += 'train loss' + ': ' + des % loss
    ...
    def _train_epoch(self, train_dl, test_dl, epoch_idx, show_progress=True):
        ...
        for batch_idx, batch_data in enumerate(iter_data):
            if self.loss_type == 'll':
                ...
                x_session = None
                session_len = None
                x, y,x_session,user_hist,hist_labels,cur_session,hist_sessions = batch_data
                ...
                if self.model.use_hist:
                    if self.config['have_es']:
                        ...
                        session_len = torch.where(session_len>0, session_len,session_len+1 )
                        # 每个session有效长度, 形状（sample_num,session_num）
                        hist_sessions_len2 = torch.sum(hist_sessions[:,:,:,0] !=0, axis=2)
                        # 历史session个数, 形状（sample_num,）
                        hist_sessions_len = torch.sum(hist_sessions_len2 > 0, axis=1)
                        ...
                        hist_sessions_len = hist_sessions_len.to(self.device)
                        hist_sessions_len2 = hist_sessions_len2.to(self.device)
                ...
                if self.model.get_name() == 'MODEM':
                    ...
                    compare_loss = CompareLoss(margin=self.margin)
                    contrast_loss = InfoNCE(tau=self.tau)
                    # 对每个head分别计算compare loss，取平均
                    # score (batch_size, num_heads, len)
                    num_heads = ll_score.shape[1]
                    sub_scores_ll = [ll_score[:, i, :] for i in range(num_heads)]
                    sub_scores_ls = [ls_score[:, i, :] for i in range(num_heads)]
                    sub_scores_ss = [ss_score[:, i, :] for i in range(num_heads)]
                    sub_scores_sl = [sl_score[:, i, :] for i in range(num_heads)]
                    sub_scores_contrast = [contrast_score[:,i,:] for i in range(num_heads)]
                    compare_loss_res = 0
                    contrast_loss_res = 0
                    for i in range(num_heads):
                        # print("hist_sessions_len: ",hist_sessions_len.max())
                        compare_loss_res += compare_loss(sub_scores_ll[i],sub_scores_ls[i],sub_scores_ss[i],sub_scores_sl[i],hist_sessions_len)
                        contrast_loss_res += contrast_loss(sub_scores_contrast[i])

                    compare_loss_res /= num_heads
                    contrast_loss_res /= num_heads

                    loss = self.lamda1*main_loss + self.lamda2*compare_loss_res + self.lamda3*contrast_loss_res
                    # print("\nloss(main,compare,contrast): ",main_loss,compare_loss_res,contrast_loss_res)
                    # loss = main_loss


                elif self.train_mode == 'refine' and score1 is not None:
        ...
        for batch_idx, batch_data in enumerate(iter_data):
            ...
            if self.model.use_hist:
                if self.config['have_es']:
                    ...
                    x_session,session_len, user_hist, hist_len,cur_session, hist_sessions,hist_sessions_len,hist_sessions_len2 = self._get_user_hist(hist_indexs,'test')
    ...
    def fit(self, train_dl, test_dl=None, verbose=True, saved=True, show_progress=True,ckpt_file = None):
        ...
        for epoch_idx in range(self.start_epoch, self.max_epochs):
            ...
            # eval
            # if epoch_idx>=10 and (epoch_idx + 1) % self.eval_step == 0 or epoch_idx<10 and (epoch_idx+1)%5==0:
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                continue_metric_output = ("epoch %d evaluating" + " [" + "time"
                                    + ": %.2fs, " + self.continue_metric + ": %f]") % \
                ...
                if save_flag and saved:
                    ...
                    save_output = 'Saving current best' + ': %s' % self.saved_model_file
                    if verbose:
