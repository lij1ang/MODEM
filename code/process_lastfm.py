...
def add_hyper_user_id():
    ...
    data = pd.read_csv(os.path.join(DATA_DIR,'remapped_tabular.csv'))
    ...
    # 添加duration,统计历史出现次数作为duration, 先按照uid（后续可调为huid）
    # 只统计训练数据上的duration
    df = data_new.sort_values(by="time",ascending=True)[:int(0.8*len(data_new))].groupby(['uid','item']).count()['time']
    uid_item_to_freq =  df.to_dict()
    data_new['duration'] = data_new.set_index(['uid', 'item']).index.map(uid_item_to_freq.get)
    data_new['duration'] = data_new['duration'].fillna(0).astype(int)
...
def generate_all_user_history():
    ...
    user_hist = train_data[['hyper_user_id','item','uid','time','duration']]
...
def generate_pos_train_data():
    ...
    sn_idx, time_idx, item_idx,author_idx, user_idx, duration_idx = 0,1,3,5,-2,-1
    BEHAVIOR_LENGTH = 40
    max_session_num = 100  # use recent 100 sessions
    cur_session, hist_sessions=[], []
    x_train ,x_session, x_behavior,y_train =[], [], [],[]
    for uid, sessions in tqdm(user_session_dict.items()):
        if uid not in hist_train:
            continue
        all_behaviors = hist_train[uid]
        hist_sessions_accumulate = [] #present之前所有session
        for i in range(1,min(len(sessions)-1,max_session_num)):
            ...
            for j in range(1, len(present_session)):    
                ...
                last_session_behavior_with_duration = []
                ...
                for k in range(len(last_session)):
                    ...
                    last_session_behavior_with_duration.append([behavior[item_idx]]+item_dict[behavior[item_idx]]+[behavior[sn_idx]]+[behavior[duration_idx]])
                ...
                cur_session.append(last_session_behavior_with_duration)
                ...
                y_train.append(1)
                hist_sessions.append(hist_sessions_accumulate)

            # 每次添加当前session到present
            selected_columns = []
            for array in present_session:
                # 从每个数组中按照指定的索引顺序选取元素
                selection = array[[item_idx, author_idx, sn_idx, duration_idx]]
                # 将选取的结果添加到列表中
                selected_columns.append(selection)
            selected_columns = np.array(selected_columns)
            hist_sessions_accumulate.append(selected_columns)

    with open(os.path.join(DATA_DIR,'train_all_pos_data.pkl'),'wb') as f:
        pkl.dump([x_train,y_train,x_session,x_behavior,cur_session,hist_sessions],f)
...
def train_data_padding():
    ...
    with open(os.path.join(DATA_DIR,'train_all_pos_data.pkl'),'rb') as f:
        x_train,y_train,x_session,x_behavior,cur_session,hist_sessions = pkl.load(f)

    HIST_SESSIONS_LENGTH = 10 # 待调整
    SESSION_LENGTH = 20
    BEHAVIOR_LENGTH = 40
    def padding(x,length,fill_num = 3):
        if len(x) >=length:
            return np.array(x[-length:])
        elif len(x) ==0 :
            return np.array([[0]*fill_num]*length)
        else:
            x = np.concatenate([x,[[0]*fill_num]*(length-len(x))])
            return x
        
    # （l1,l2,4）==> (hist_sessions_length,session_length,5), 填充结果待验证
    def padding_hist_sessions(x,length1,length2):
        # 先填充第二维 (l1,l2,4) ==> (l1,session_length,4)
        for i in range(len(x)):
            x[i] = padding(x[i],length2,fill_num = 4)

        # 填充第一维 (l1,session_length,4) ==> (hist_sessions_length,session_length,4)
        if len(x) >= length1:
            return np.array(x[-length1:])
        elif len(x) == 0:
            return np.array([[[0]*4]*length2]*length1)
        else:
            return np.concatenate([x,[[[0]*4]*length2]*(length1-len(x))])

    cnt=0
    good_cnt=0
    x_session_pos_train_with_one_snid = []
    x_behavior_pos_train_with_one_snid = []
    cur_session_pos_train_with_one_snid = []
    hist_sessions_pos_train_with_one_snid = []
    x_pos_train_with_one_snid = []
    y_pos_train_with_one_snid = []
    x_session_pos_train_with_two_snid = []
    x_behavior_pos_train_with_two_snid = []
    cur_session_pos_train_with_two_snid = []
    hist_sessions_pos_train_with_two_snid = []
    ...
    for i in tqdm(range(len(x_behavior))):
        if(sum(x_behavior[i][:,-1] !=x_behavior[i][0][-1])==0):
            ...
            cur_session_pos_train_with_one_snid.append(padding(cur_session[i],SESSION_LENGTH,4))
            hist_sessions_pos_train_with_one_snid.append(padding_hist_sessions(hist_sessions[i],HIST_SESSIONS_LENGTH,SESSION_LENGTH))
    ...
    with open(os.path.join(DATA_DIR,'train_pos_data_with_one_snid.pkl'),'wb') as f:
        pkl.dump([x_pos_train_with_one_snid,y_pos_train_with_one_snid,x_session_pos_train_with_one_snid,x_behavior_pos_train_with_one_snid
                  ,cur_session_pos_train_with_one_snid,hist_sessions_pos_train_with_one_snid],f)
        
    del x_session_pos_train_with_one_snid,x_behavior_pos_train_with_one_snid,cur_session_pos_train_with_one_snid,hist_sessions_pos_train_with_one_snid,\
    x_pos_train_with_one_snid,y_pos_train_with_one_snid
    gc.collect()
    ...
    for i in tqdm(range(len(x_behavior))):
        if(sum(x_behavior[i][:,-1] !=x_behavior[i][0][-1])>0):
            ...
            cur_session_pos_train_with_two_snid.append(padding(cur_session[i],SESSION_LENGTH,4))
            hist_sessions_pos_train_with_two_snid.append(padding_hist_sessions(hist_sessions[i],HIST_SESSIONS_LENGTH,SESSION_LENGTH))
    ...
    with open(os.path.join(DATA_DIR,'train_pos_data_with_two_snid.pkl'),'wb') as f:
        pkl.dump([x_pos_train_with_two_snid,y_pos_train_with_two_snid,x_session_pos_train_with_two_snid,x_behavior_pos_train_with_two_snid
                  ,cur_session_pos_train_with_two_snid,hist_sessions_pos_train_with_two_snid],f)
...
def generate_train_set(train_set_pos_file,train_set_file):
    ...
    with open(os.path.join(DATA_DIR, train_set_pos_file),'rb') as f:
        x_pos_train,y_pos_train,x_session_pos_train,x_behavior_pos_train,cur_session_pos_train,hist_sessions_pos_train = pkl.load(f)

    TRAIN_SIZE = 200000
    x_pos_train,y_pos_train,x_session_pos_train,x_behavior_pos_train,cur_session_pos_train,hist_sessions_pos_train = \
    x_pos_train[-TRAIN_SIZE:],y_pos_train[-TRAIN_SIZE:],x_session_pos_train[-TRAIN_SIZE:],x_behavior_pos_train[-TRAIN_SIZE:],cur_session_pos_train[-TRAIN_SIZE:],hist_sessions_pos_train[-TRAIN_SIZE:]
    ...
    for i in tqdm(range(len(x_pos_train))):
        ...
        for idx in idxs:
            ...
            if neg_item[0] not in pos_aids:
                ...
                cur_session_neg.append(cur_session_pos_train[i])
                hist_sessions_neg.append(hist_sessions_pos_train[i])
            else: 
                while True:
    ...
    cur_session_neg = np.array(cur_session_neg)
    hist_sessions_neg = np.array(hist_sessions_neg)
    ...
    train_cur_session = np.concatenate((cur_session_pos_train, cur_session_neg), axis=0)
    train_hist_sessions = np.concatenate((hist_sessions_pos_train, hist_sessions_neg), axis=0)
    print(train_x.shape)
    print(train_y.shape)
    print(train_x_session.shape)
    print(train_x_behavior.shape)
    print(train_cur_session.shape)
    print(train_hist_sessions.shape)
    
    
    with open(os.path.join(DATA_DIR, 'input_data', train_set_file), 'wb') as f:
        pkl.dump([train_x, train_y,train_x_session,train_x_behavior,train_cur_session,train_hist_sessions], f,protocol=4)
    
    print('Train data is generated')

def convert_train_type(train_set_file):
    with open(os.path.join(DATA_DIR, 'input_data', train_set_file),'rb') as f:
        train_x, train_y,train_x_session,train_x_behavior,hist_labels,train_cur_session,train_hist_sessions = pkl.load(f)

    train_x, train_y,train_x_session,train_x_behavior,train_cur_session,train_hist_sessions = \
        train_x.astype('int32'), train_y.astype('int32'),train_x_session.astype('int32'),train_x_behavior.astype('int32'),train_cur_session.astype('int32'),train_hist_sessions.astype('int32')
    with open(os.path.join(DATA_DIR, 'input_data', train_set_file), 'wb') as f:
        pkl.dump([train_x, train_y,train_x_session,train_x_behavior,hist_labels,train_cur_session,train_hist_sessions], f,protocol=4)
...
def generate_pos_test_data():
    ...
    cur_session_test, hist_sessions_test=[], []
    ...
    for uid, sessions in tqdm(test_user_session_dict.items()):
        ...
        for i in range(1,min(len(sessions),max_session_num)):
            ...
            for j in range(1, len(present_session)):    
                ...
                cur_session_test.append(last_session_behavior_with_duration)
                ...
                y_test.append(1)
                hist_sessions_test.append(hist_sessions_accumulate)
    ...
    with open(os.path.join(DATA_DIR,'test_all_pos_data.pkl'),'wb') as f:
        pkl.dump([x_test,y_test,x_session_test,x_behavior_test,cur_session_test,hist_sessions_test],f)
...
def test_data_padding():
    ...
    with open(os.path.join(DATA_DIR,'test_all_pos_data.pkl'),'rb') as f:
        x_test,y_test,x_session_test,x_behavior_test,cur_session_test,hist_sessions_test = pkl.load(f)
    
    HIST_SESSIONS_LENGTH = 10
    SESSION_LENGTH = 30
    BEHAVIOR_LENGTH = 50
    ...
    for i in tqdm(range(len(x_behavior_test))):
        ...
        cur_session_test[i] = padding(cur_session_test[i],SESSION_LENGTH,4)
        hist_sessions_test[i] = padding_hist_sessions(hist_sessions_test[i],HIST_SESSIONS_LENGTH,SESSION_LENGTH)
...
def generate_test_set():
    ...
    with open(os.path.join(DATA_DIR, 'test_pos_data.pkl'),'rb') as f:
        x_test,y_test,x_session_pos_test,x_behavior_pos_test,cur_session_pos_test,hist_sessions_pos_test = pkl.load(f)
    ...
    for i in tqdm(range(len(x_test))):
        ...
        test_hist_dict[i] = [x_session_pos_test[i],x_behavior_pos_test[i]
                             ,cur_session_pos_test[i],hist_sessions_pos_test[i]]
...
def convert_test_type():
    with open(os.path.join(DATA_DIR, 'input_data','test_hist_dict.pkl'), 'rb') as f:
        test_hist_dict = pkl.load(f)
    ...
    with open(os.path.join(DATA_DIR, 'input_data','test_set_small.pkl'), 'rb') as f:
        test_x_small,test_y_small,hist_indexs_small = pkl.load(f)

    test_x,test_y,hist_indexs,test_x_small,test_y_small,hist_indexs_small = \
        test_x.astype('int32'),test_y.astype('int32'),hist_indexs.astype('int32'),test_x_small.astype('int32'),test_y_small.astype('int32'),hist_indexs_small.astype('int32')
    
    for i in test_hist_dict.keys():
        test_hist_dict[i] = [x.astype('int32') for x in test_hist_dict[i]]

    with open(os.path.join(DATA_DIR, 'input_data','test_hist_dict.pkl'), 'wb') as f:
        pkl.dump(test_hist_dict,f)
    with open(os.path.join(DATA_DIR, 'input_data','test_set.pkl'), 'wb') as f:
        pkl.dump([test_x,test_y,hist_indexs],f,protocol=4)
    with open(os.path.join(DATA_DIR, 'input_data','test_set_small.pkl'), 'wb') as f:
        pkl.dump([test_x_small,test_y_small,hist_indexs_small],f,protocol=4)

def get_small_test_set(EVAL_SIZE=2000*1024):
    with open(os.path.join(DATA_DIR, 'input_data','test_set.pkl'), 'rb') as f:
        test_x,test_y,hist_indexs = pkl.load(f)

    with open(os.path.join(DATA_DIR, 'input_data','test_set_small_2000.pkl'), 'wb') as f:
        pkl.dump([test_x[-EVAL_SIZE:],test_y[-EVAL_SIZE:],hist_indexs[-EVAL_SIZE:]],f,protocol=4)
...
def generate_hist_labels(train_set_file,train_set_file_with_hist_labels):
    ...
    with open(os.path.join(DATA_DIR, 'input_data', train_set_file), 'rb') as f:
        train_x, train_y,train_x_session,train_x_behavior,cur_session,hist_sessions = pkl.load(f)
    ...
    hist_labels = []
    # print("\n print_length: ",len(session_sns),len(hist_sns))
    ...
    with open(os.path.join(DATA_DIR, 'input_data', train_set_file_with_hist_labels), 'wb') as f:
        pkl.dump([train_x, train_y,train_x_session,train_x_behavior, hist_labels, cur_session, hist_sessions], f,protocol=4)
...
def join_train_files():
    ...
    with open(os.path.join(DATA_DIR, 'input_data', 'train_set_with_one_snid_and_hist_labels.pkl'), 'rb') as f:
        train_x1, train_y1,train_x_session1,train_x_behavior1,hist_labels1,cur_session_1,hist_sessions_1 = pkl.load(f)

    with open(os.path.join(DATA_DIR, 'input_data', 'train_set_with_two_snid_and_hist_labels.pkl'), 'rb') as f:
        train_x2, train_y2,train_x_session2,train_x_behavior2,hist_labels2,cur_session_2,hist_sessions_2 = pkl.load(f)
    ...
    cur_session = np.concatenate([cur_session_1,cur_session_2])
    hist_sessions = np.concatenate([hist_sessions_1,hist_sessions_2])

    
    with open(os.path.join(DATA_DIR, 'input_data', 'train_set_all.pkl'), 'wb') as f:
        pkl.dump([train_x,train_y,train_x_session,train_x_behavior,hist_labels,cur_session,hist_sessions],f)
...
if __name__ == '__main__':
    ...
    # convert_train_type('train_set_with_two_snid_and_hist_labels.pkl')
    # convert_train_type('train_set_with_one_snid_and_hist_labels.pkl')
    # convert_test_type()
    # get_small_test_set()
