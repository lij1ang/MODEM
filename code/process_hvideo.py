...
# 添加带有duration的cur session和hist session
def generate_pos_train_data():
    ...
    BEHAVIOR_LENGTH = 50
    max_session_num = 300  # use recent 100 sessions
    x_train ,x_session, x_behavior,y_train =[], [], [],[]
    # 有duration的当前session，历史sessions
        ...
        for i in range(1,min(len(sessions)-1,max_session_num)):
            ...
            for j in range(1, len(present_session)):    
                ...
                for k in range(len(last_session)):
                    ...
                    last_session_behavior_with_duration.append([behavior[item_idx]]+[behavior[sn_idx]]+[behavior[duration_idx]])
            ...
            hist_sessions_accumulate.append(present_session)
...
def train_data_padding(behavior_length = 20):
    ...
    HIST_SESSIONS_LENGTH = 10 # user平均session个数13.72，样本平均session个数应/2，约为6.8
    SESSION_LENGTH = 10
    BEHAVIOR_LENGTH = behavior_length
    def padding(x,length,fill_num=2):
        if len(x) >=length:
    ...
    def padding_hist_sessions(x,length1,length2):
        # 先填充第二维 (l1,l2,5) ==> (l1,session_length,5)
        for i in range(len(x)):
            x[i] = padding(x[i],length2,fill_num = 5)

        # 填充第一维 (l1,session_length,5) ==> (hist_sessions_length,session_length,5)
        if len(x) >= length1:
            return np.array(x[-length1:])
        elif len(x) == 0:
            return np.array([[[0]*5]*length2]*length1)
        else:
            return np.concatenate([x,[[[0]*5]*length2]*(length1-len(x))])
    ...
    for i in tqdm(range(len(x_behavior))):
        if(sum(x_behavior[i][-BEHAVIOR_LENGTH:,-1] !=x_behavior[i][0][-1])==0):
            ...
            cur_session_pos_train_with_one_snid.append(padding(cur_session[i],SESSION_LENGTH,3))
            ...
            cur_session_pos_train_with_two_snid.append(padding(cur_session[i],SESSION_LENGTH,3))
...
def generate_pos_test_data():
    ...
    for uid, sessions in tqdm(test_user_session_dict.items()):
        ...
        for i in range(1,min(len(sessions),max_session_num)):
            ...
            for j in range(1, len(present_session)):    
                ...
                # start_time一定晚于hist_train中所有行为，也就晚于all_behavior中所有行为
                start_time = present_session[j][time_idx]
    ...
    with open(os.path.join(DATA_DIR,'test_all_pos_data.pkl'),'wb') as f:
        pkl.dump([x_test,y_test,x_session_test,x_behavior_test,cur_session,hist_sessions],f)
    
    print('Test data generated.')    

    
def test_data_padding(behavior_length = 20):
    ...
    for i in tqdm(range(len(x_behavior_test))):
        ...
        cur_session_test[i] = padding(cur_session_test[i],SESSION_LENGTH,3)
    ...
    with open(os.path.join(DATA_DIR,'test_pos_data_w_hist_len_{}.pkl'.format(behavior_length)),'wb') as f:
        pkl.dump([x_test,y_test,x_session_test,x_behavior_test
                  ,cur_session_test,hist_sessions_test],f)


def generate_test_set(padded_length = 20):
...
def join_train_files(padded_length):
    ...
    with open(os.path.join(DATA_DIR, 'input_data', 'train_set_with_one_snid_and_hist_labels_w_hist_len_{}.pkl'.format(padded_length)), 'rb') as f:
        train_x1, train_y1,train_x_session1,train_x_behavior1,hist_labels1,cur_session_1,hist_sessions1 = pkl.load(f)
    ...
    with open(os.path.join(DATA_DIR, 'input_data', 'train_set_with_two_snid_and_hist_labels_w_hist_len_{}.pkl'.format(padded_length)), 'rb') as f:
        train_x2, train_y2,train_x_session2,train_x_behavior2,hist_labels2,cur_session_2,hist_sessions2 = pkl.load(f)
    ...
    hist_sessions = np.concatenate([hist_sessions1, hist_sessions2])
...
def generate_different_hist_len():
    ...
    for padded_length in tqdm([50,40]):
...
if __name__ == '__main__':
    ...
    # generate_train_set('train_pos_data_with_two_snid_w_hist_len_20.pkl','train_set_with_two_snid.pkl')
    # generate_train_set('train_pos_data_with_one_snid_w_hist_len_20.pkl','train_set_with_one_snid.pkl')
    # generate_pos_test_data()
    # test_data_padding()
    # generate_test_set()
